...
PRETRAINED_WEIGHTS = os.path.join(CKPT_DIR, "ckpt_best.pth")
...
# 미리 학습된 가중치 로드 (Fine-tuning 시작)
# PRETRAINED_WEIGHTS = './experiments/location_fix1_kamofinetune/ckpt/ckpt_best.pth'
init_epoch = 0  # 에폭 값을 0으로 초기화
best_metric = 0
best_distance = 1000

if os.path.exists(PRETRAINED_WEIGHTS):
    logging.info(f'Pretrained weights are being loaded from {PRETRAINED_WEIGHTS}')
    checkpoint = torch.load(PRETRAINED_WEIGHTS)

    # 가중치만 로드
    if 'network' in checkpoint:
        network.load_state_dict(checkpoint['network'])
        logging.info('Pretrained model loaded. Starting fine-tuning.')
    else:
        logging.error(f'Checkpoint does not contain network weights.')

else:
    logging.error(f'Pretrained weights not found at {PRETRAINED_WEIGHTS}')
...
early_stop_patience = 5  # 성능 개선이 없을 때 멈추기 위한 최대 에포크 수
